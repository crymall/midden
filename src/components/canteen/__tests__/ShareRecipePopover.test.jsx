...
// Mock for Headless UI Combobox issues in test env
...
describe("ShareRecipePopover", () => {
  ...
  describe("Popover Behavior", () => {
    ...
    it("opens popover, copies link, and displays copied state", async () => {
      ...
      // Assert link buttons are present
      ...
      const expectedUrl = `http://localhost:3000/applications/canteen/recipes/${mockRecipe.id}`;
  ...
  describe("Sharing Modal Interaction", () => {
    it("opens modal on friend selection, sends message, and closes", async () => {
      ...
      await act(async () => {
        fireEvent.keyDown(searchInput, { key: "Enter", code: "Enter" }); // Emulates ListAddPopover
      });
      ...
      // Crucial step: wait for the modal to disappear to ensure the state completely settles
      // (catches the finally { setSending(false) } block before the test exits)
      await waitFor(() => {
